        ...
        */
        ...
        public static byte[] ToWav(byte[] adxData)
        {
            ...
            using (BinaryWriter wavWriter = new BinaryWriter(wavStream))
            {
                ...
                int channel = buf[7];
                int freq = Utils.ReadUInt32BE(buf, 8);
                int size = Utils.ReadUInt32BE(buf, 12);

                int offset = Utils.ReadUInt16BE(buf, 2) - 2;
        ...
        /// <param name="loop">If you need your whole file to loop. Default to False.</param>
        /// <returns></returns>
        /// <exception cref="InvalidOperationException"></exception>
        public static byte[] FromWav(byte[] wavData, bool loop = false)
        {
            ...
            using (BinaryWriter adxWriter = new BinaryWriter(adxStream))
            {
                ...
                Utils.WriteUInt32BE(finalBlock, 0, 0x8001000e);
        ...
        private static void WriteAdxHeader(BinaryWriter adxWriter, int channel, int freq, int size, bool loop)
        {
            ...
            Utils.WriteUInt32BE(adxhdr, 8, freq);
            Utils.WriteUInt32BE(adxhdr, 12, size);
            Utils.WriteUInt16BE(adxhdr, 16, ADX_HIGHPASS_FREQ);
            adxhdr[18] = ADX_VERSION;
            adxhdr[19] = ADX_FLAGS;
            if (loop)
            {
                offset = LOOP_START_OFFSET - 4;
                Utils.WriteUInt16BE(adxhdr, 20, 0);
                Utils.WriteUInt16BE(adxhdr, 22, 1);
                Utils.WriteUInt32BE(adxhdr, 24, 1);
                Utils.WriteUInt32BE(adxhdr, 28, 0);
                Utils.WriteUInt32BE(adxhdr, 32, LOOP_START_OFFSET);
                Utils.WriteUInt32BE(adxhdr, 36, size);
                int loopEndOffset = GetLoopEndOffset(LOOP_START_OFFSET, size);
                Utils.WriteUInt32BE(adxhdr, 40, loopEndOffset);
            }
            Utils.WriteUInt32BE(adxhdr, 0, offset | 0x80000000);
